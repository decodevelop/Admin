...
class IncidenciasNewController extends Controller
{
  ...
  */
  ...
  private function guardar_reposicion($id,$productos_incidencia){
    ...
    // Obtenemos el cliente y su direccion
    $cliente = Clientes_pedidos::find($pedido_base->cliente->id);
    $direccion = Direcciones::find($pedido_base->id_direccion);

    if(is_null($direccion)){
      $direccion = Direcciones::find($pedido_base->cliente->direcciones[0]->id);
    }
