...
public function actualizar($id, Request $request){
  ...
  if(is_null($pedido->direccion)){
    $direccion = $pedido->cliente->direcciones[0];
  } else {
...
public function csv_mrw($id,$generar_csv){
  ...
  if(is_null($pedido->direccion)){
    if($pedido->cliente->direcciones[0]->pais_envio == ""){
      $pais_fact = "ES";
    }else{
      $pais_fact = $pedido->cliente->direcciones[0]->pais_envio;
    }
  }else {
    if($pedido->direccion->pais_envio == ""){
      $pais_fact = "ES";
    }else{
      $pais_fact = $pedido->direccion->pais_envio;
    }
  }
  ...
  if(is_null($pedido->direccion)){
    ...
    'direccion' => ''.$pedido->cliente->direcciones[0]->direccion_envio.'',
    'cp' => ''.trim($pedido->cliente->direcciones[0]->cp_envio).'',
    'poblacion' => ''.$pedido->cliente->direcciones[0]->ciudad_envio.'',
  ...
  'observacion' => ''.$pedido->observaciones.'',
  'nombre_apellido' => ''.$pedido->cliente->nombre_envio.'',
  'direccion' => ''.$pedido->direccion->direccion_envio.'',
  'cp' => ''.trim($pedido->direccio->cp_envio).'',
  'poblacion' => ''.$pedido->direccion->ciudad_envio.'',
  'codigo_pais' => $pais_fact,
  'telefono' => ''.$tlf.'',
  'franquicia' => '',
  'adicionales' => ''.$datos_adicionales.''
...
public function csv_tipsa($id,$generar_csv){
  ...
  'nombre_apellido' => ''.$pedido->cliente->nombre_envio.'',
  'direccion' => ''.$pedido->direccion->direccion_envio.'',
  'cp' => ''.trim($pedido->direccion->cp_envio).'',
  'poblacion' => ''.$pedido->direccion->ciudad_envio.'',
  'codigo_pais' => $pais_fact,
  'telefono' => ''.$tlf.'',
  'franquicia' => '',
...
public function csv_szendex($id,$generar_csv){
  ...
  if(is_null($pedido->direccion)){
    $pais = $pedido->cliente->direcciones[0]->pais_envio;
  }else {
    $pais = $pedido->direccion->pais_envio;
  }
      ...
      foreach($ids as $key => $id){
        ...
        if(is_null($pedido->direccion)){
          $product_excel = array(
            ...
            $pedido->cliente->direcciones[0]->direccion_envio
          );
        } else {
          $product_excel = array(
            $pedido->id,
            ...
            $pedido->cliente->email,
            $pedido->cliente->telefono,
            $pedido->direccion->direccion_envio
          );
        }
